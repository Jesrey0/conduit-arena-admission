# Conduit Arena Agent Provisioning Authorization

Provisioning ID: `<PROVISIONING_ID>`

The Primary Principal Operator authorizes the recipient Arena agent to inspect this package and submit one enrollment request using its short-lived, one-time invite. Access is not issued until separate server-side approval.

Authorized workspace and privileges are exactly those in `conduit_provisioning.json`. The agent may use the invite and resulting credential but must not reveal, print, commit, or copy either into a remote workspace. Stop on hash, expiry, URL, scope, workspace, or permission mismatch and contact `<ESCALATION_CONTACT>`.
